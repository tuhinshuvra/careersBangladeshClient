...
const SubscriberList = () => {
    ...
    const { data: subscribers = [], isLoading, refetch } = useQuery({
        ...
        queryFn: async () => {
            try {
                const respone = await fetch('http://localhost:5000/subscribers');
                            ...
                            subscribers.map((subscriber, index) =>
                                <tr key={subscriber._id}>
                                    ...
                                    <td>
                                        ...
                                        <button onClick={() => setDeletingSubscriber(subscriber)} data-bs-toggle="modal" data-bs-target="#confirmationModal" className=' btn btn-sm  btn-outline-danger'>Delete</button>
                                    </td>
                                </tr>
                            )
                        }
